...
export function RevenueCharts({
  ...
  const marketerTierData = useMemo(() => {
    ...
    selectedMonthDeals.forEach(deal => {
      ...
      const existing = marketerMap.get(email) || {
        email,
        netRevenue: 0
      };
    ...
    marketerMap.forEach(({
      ...
      const {
        tier,
        commission
      } = calculateTieredCommission(netRevenue);
      data.push({
        marketer: email,
        netRevenue,
        tier,
        ...
        <div className="h-[250px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={monthlyData}>
              ...
              <Area type="monotone" dataKey="net" name="Net Revenue" stroke="hsl(var(--primary))" fillOpacity={1} fill="url(#colorNet)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div className="h-[250px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={monthlyData}>
              ...
              <Bar dataKey="fees" name="Fees" fill="hsl(var(--destructive))" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
